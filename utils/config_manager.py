...
def save_config(filename: str, menu_cfg, aim_cfg, trig_cfg, radar_cfg) -> bool:
    ...
    data = {
        "visuals": {
            ...
            "box_width_scale":       menu_cfg.box_width_scale,
            "box_height_scale":      menu_cfg.box_height_scale,
            ...
            "player_name_color":     menu_cfg.player_name_color,
            "show_distance":         menu_cfg.show_distance,
            "distance_color":        menu_cfg.distance_color,
        ...
        "aimbot": {
            ...
            "target_lock":        aim_cfg.target_lock,
            "shot_timing":        aim_cfg.shot_timing,
            "shot_threshold":     aim_cfg.shot_threshold,
            "spray_control":      aim_cfg.spray_control,
            "crouch_pred":        aim_cfg.crouch_pred,
        },
        "triggerbot": {
...
def load_config(filename: str, menu_cfg, aim_cfg, trig_cfg, radar_cfg) -> bool:
    # Uzantisiz veya uzantili gelebilir
    ...
    menu_cfg.box_width_scale     = float(v.get("box_width_scale",  menu_cfg.box_width_scale))
    menu_cfg.box_height_scale    = float(v.get("box_height_scale", menu_cfg.box_height_scale))
    ...
    menu_cfg.player_name_color   = v.get("player_name_color",  menu_cfg.player_name_color)
    menu_cfg.show_distance       = v.get("show_distance",      menu_cfg.show_distance)
    menu_cfg.distance_color      = v.get("distance_color",     menu_cfg.distance_color)
    ...
    aim_cfg.target_lock          = a.get("target_lock",        aim_cfg.target_lock)
    aim_cfg.shot_timing          = a.get("shot_timing",        aim_cfg.shot_timing)
    aim_cfg.shot_threshold       = a.get("shot_threshold",     aim_cfg.shot_threshold)
    aim_cfg.spray_control        = a.get("spray_control",      aim_cfg.spray_control)
    aim_cfg.crouch_pred          = a.get("crouch_pred",        aim_cfg.crouch_pred)
...
def load_last_config(menu_cfg, aim_cfg, trig_cfg, radar_cfg) -> bool:
    ...
    try:
        ...
        check = last if last.endswith(".json") else last + ".json"
        if not os.path.exists(os.path.join(CONFIG_DIR, check)):
            print(f"[ config ] Son config bulunamadi: {check}, atlanıyor.")
            # Gecersiz _last.txt temizle
            try:
                os.remove(LAST_CFG_FILE)
            except Exception:
                pass
            return False
        return load_config(last, menu_cfg, aim_cfg, trig_cfg, radar_cfg)
    except Exception as e:
        print(f"[ config ] load_last_config hatasi: {e}")
        return False


def list_configs() -> list:
    """config/ klasoründeki tum .json dosyalarini uzantisiz listele."""
    _ensure_dir()
    return sorted([
