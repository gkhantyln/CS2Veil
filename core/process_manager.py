...
# Thread-local buffer storage — externalv2 mimarisi, her thread kendi buffer'ını tutar
_tls = threading.local()

def _get_buf(size: int) -> ctypes.Array:
    cache = getattr(_tls, "cache", None)
    if cache is None:
        _tls.cache = {}
        cache = _tls.cache
    if size not in cache:
        cache[size] = (ctypes.c_char * size)()
    return cache[size]

def _get_bytes_read() -> ctypes.c_size_t:
    br = getattr(_tls, "bytes_read", None)
    if br is None:
        _tls.bytes_read = ctypes.c_size_t(0)
    return _tls.bytes_read
...
class ProcessManager:
    def __init__(self):
        ...
        self.pid: int = 0
        # Paylaşılan buffer'lar kaldırıldı — thread-local _get_buf() kullanılıyor
    ...
    def read_memory(self, address: int, size: int) -> Optional[bytes]:
        ...
        ok  = kernel32.ReadProcessMemory(
            ...
            buf, size, ctypes.byref(rc)
        )
        return bytes(buf[:rc.value]) if ok and rc.value > 0 else None
